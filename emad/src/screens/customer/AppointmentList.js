                        ...
                        <View key={item.id} style={{ height: 75, width: "90%", flexDirection: "row", alignSelf: "center", marginTop: 10, marginBottom: 5, }}>
                            ...
                            <TouchableOpacity style={{ flexDirection: 'row', width: '75%' }} activeOpacity={.75} onPress={() => { navigation.navigate('CustomerPage', { cliente: item.id_cliente }) }}>
                                <View style={{ flexDirection: "column", justifyContent: "center" }}>
                                    ...
                                    <Text style={{ fontSize: 12, fontFamily: 'SFProDisplayRegular', color: colors.theme.title, justifyContent: 'space-around'}}>
                                        {slots.find(slots => slots.value == item.slot_inizio).slot}
                                        {" - "}
                                        {slots.find(slots => slots.value == item.slot_fine).slot}
                                        {"   "}
